...
export const data: Object[] = orderData.map((item: cType) => {
  const name: cType = (customerData as cType[]).filter((cItem: cType) => {
    return cItem.CustomerID === item.CustomerID;
  })[0];
  item.CustomerName = (name || ({} as cType)).ContactName;
  return item;
});
...
export const data1: Object[] = orderDataSource.map((item: cType) => {
  const name1: cType = (customerData as cType[]).filter((cItem: cType) => {
    return cItem.CustomerID === item.CustomerID;
  })[0];
  item.CustomerName = (name1 || ({} as cType)).ContactName;
  return item;
});
/**
